...
export const createMercadoPagoPreference = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const json = (await r.json().catch(() => ({}))) as { init_point?: string; message?: string };
    if (!r.ok || !json.init_point) return { ok: false as const, error: json.message || "mp" };
    return { ok: true as const, url: json.init_point };
  });
